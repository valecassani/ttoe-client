...
const Game = ({ match }) => {
    ...
    const buildRow = (extIndex, dimension, moves) => {
        ...
        return arr.fill().map((_, index) => (
            <Box 
                color="text.primary" 
                style={{minHeight: '40px', minWidth: '40px', flexGrow: 1, border: '1px white solid', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }} 
    ...
    const buildGrid = (dimension, moves) => {
        ...
        return arr.fill(1).map((_, index) => {
            return <div style={{ display: 'flex', flexDirection: 'row', flexGrow: 1}}>{buildRow(index, dimension, moves)}</div>;
        })
    }
